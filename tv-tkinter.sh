...
SPECIFIED_FOLDER = "/data/data/com.termux/files/home/play"
...
class VLCQuadrantsApp:
    ...
    def setup_ui(self):
        ...
        ttk.Label(control_frame, text="Select URL File:").pack(side=tk.LEFT, padx=5)
        ...
        self.file_dropdown.bind("<<ComboboxSelected>>", self.load_urls)
        self.populate_dropdown()
        self.file_dropdown.focus_set()
        # Video Grid (2x2)
    ...
    def populate_dropdown(self):
        """Populate dropdown with .txt files."""
        if os.path.exists(SPECIFIED_FOLDER):
            files = [f for f in os.listdir(SPECIFIED_FOLDER) if f.endswith('.txt')]
            self.file_dropdown['values'] = files

    def load_urls(self, event=None):
        """Read 4 URLs and play them."""
        filepath = os.path.join(SPECIFIED_FOLDER, self.file_var.get())
